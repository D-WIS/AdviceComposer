    ...
    public class Worker : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken cancellationToken)
        {
            ...
            if (_DWISClient != null && _DWISClient.Connected)
            {
                ...
                queries = GeneratorSparQLManifestFile.GetSparQLQueries(assembly, typeof(ADCSStandardAutoDrillerDrillStemVibrationFeatureHelper).FullName, "BOSFlowrateAdvisedTarget");
                ManifestFile? manifest = GeneratorSparQLManifestFile.GetManifestFile(assembly, typeof(ADCSStandardAutoDrillerDrillStemVibrationFeatureHelper).FullName, "BOSFlowrateAdvisedTarget", _manifestName, _companyName, _prefix + "BOSFlowrateAdvisedTarget");
                ...
                queries = GeneratorSparQLManifestFile.GetSparQLQueries(assembly, typeof(ADCSStandardAutoDrillerDrillStemVibrationFeatureHelper).FullName, "BOSRotationalSpeedAdvisedTarget");
                manifest = GeneratorSparQLManifestFile.GetManifestFile(assembly, typeof(ADCSStandardAutoDrillerDrillStemVibrationFeatureHelper).FullName, "BOSRotationalSpeedAdvisedTarget", _manifestName, _companyName, _prefix + "BOSRotationalSpeedAdvisedTarget");
                if (queries != null && queries.Count > 0 && queries.First().Value != null && !string.IsNullOrEmpty(queries.First().Value.SparQL) && queries.First().Value.Variables != null && queries.First().Value.Variables!.Count > 0 && manifest != null)
                {
                    UpdateAdvisor(manifest, "Advisor", _advisorName);
                ...
                queries = GeneratorSparQLManifestFile.GetSparQLQueries(assembly, typeof(ADCSStandardAutoDrillerDrillStemVibrationFeatureHelper).FullName, "ROPAdvisedMaximum");
                manifest = GeneratorSparQLManifestFile.GetManifestFile(assembly, typeof(ADCSStandardAutoDrillerDrillStemVibrationFeatureHelper).FullName, "ROPAdvisedMaximum", _manifestName, _companyName, _prefix + "ROPAdvisedMaximum");
                if (queries != null && queries.Count > 0 && queries.First().Value != null && !string.IsNullOrEmpty(queries.First().Value.SparQL) && queries.First().Value.Variables != null && queries.First().Value.Variables!.Count > 0 && manifest != null)
                {
                    UpdateAdvisor(manifest, "Advisor", _advisorName);
                ...
                queries = GeneratorSparQLManifestFile.GetSparQLQueries(assembly, typeof(ADCSStandardAutoDrillerDrillStemVibrationFeatureHelper).FullName, "WOBAdvisedMaximum");
                manifest = GeneratorSparQLManifestFile.GetManifestFile(assembly, typeof(ADCSStandardAutoDrillerDrillStemVibrationFeatureHelper).FullName, "WOBAdvisedMaximum", _manifestName, _companyName, _prefix + "WOBAdvisedMaximum");
                if (queries != null && queries.Count > 0 && queries.First().Value != null && !string.IsNullOrEmpty(queries.First().Value.SparQL) && queries.First().Value.Variables != null && queries.First().Value.Variables!.Count > 0 && manifest != null)
                {
                    UpdateAdvisor(manifest, "Advisor", _advisorName);
                ...
                queries = GeneratorSparQLManifestFile.GetSparQLQueries(assembly, typeof(ADCSStandardAutoDrillerDrillStemVibrationFeatureHelper).FullName, "BitTorqueAdvisedMaximum");
                manifest = GeneratorSparQLManifestFile.GetManifestFile(assembly, typeof(ADCSStandardAutoDrillerDrillStemVibrationFeatureHelper).FullName, "BitTorqueAdvisedMaximum", _manifestName, _companyName, _prefix + "BitTorqueAdvisedMaximum");
                if (queries != null && queries.Count > 0 && queries.First().Value != null && !string.IsNullOrEmpty(queries.First().Value.SparQL) && queries.First().Value.Variables != null && queries.First().Value.Variables!.Count > 0 && manifest != null)
                {
                    UpdateAdvisor(manifest, "Advisor", _advisorName);
                ...
                queries = GeneratorSparQLManifestFile.GetSparQLQueries(assembly, typeof(ADCSStandardAutoDrillerDrillStemVibrationFeatureHelper).FullName, "DifferentialPressureAdvisedMaximum");
                manifest = GeneratorSparQLManifestFile.GetManifestFile(assembly, typeof(ADCSStandardAutoDrillerDrillStemVibrationFeatureHelper).FullName, "DifferentialPressureAdvisedMaximum", _manifestName, _companyName, _prefix + "DifferentialPressureAdvisedMaximum");
                if (queries != null && queries.Count > 0 && queries.First().Value != null && !string.IsNullOrEmpty(queries.First().Value.SparQL) && queries.First().Value.Variables != null && queries.First().Value.Variables!.Count > 0 && manifest != null)
                {
                    UpdateAdvisor(manifest, "Advisor", _advisorName);
